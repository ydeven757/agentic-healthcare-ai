...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        agent_manager = HealthcareAgentManager(
            openai_api_key=api_key, fhir_config=fhir_config
        )
...
class AssessmentRequest(BaseModel):
    ...
    patient_id: str
    assessment_type: str = (
        "comprehensive"  # comprehensive, emergency, medication_reconciliation
    )
...
async def run_comprehensive_assessment(
    ...
    try:
        logger.info(
            f"Starting comprehensive assessment for patient {request.patient_id}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Assessment failed. Check server logs for details."
        )
...
async def run_emergency_assessment(
    ...
    try:
        ...
        logger.error(
            f"Emergency assessment failed for patient {request.patient_id}: {e}"
        )
        logger.exception("Emergency assessment failed")
        raise HTTPException(
            status_code=500,
            detail="Emergency assessment failed. Check server logs for details.",
        )
...
async def run_medication_reconciliation(
    ...
    try:
        logger.info(
            f"Starting medication reconciliation for patient {request.patient_id}"
        )
        ...
        logger.error(
            f"Medication reconciliation failed for patient {request.patient_id}: {e}"
        )
        logger.exception("Medication reconciliation failed")
        raise HTTPException(
            status_code=500,
            detail="Medication reconciliation failed. Check server logs for details.",
        )
...
async def get_patient_summary(
    ...
    try:
        ...
        patient_data = await agent_manager.fhir_client.get_comprehensive_patient_data(
            patient_id
        )
...
async def run_comprehensive_assessment_compat(
    ...
    try:
        ...
        logger.error(
            f"Comprehensive assessment failed for patient {request.patient_id}: {e}"
        )
...
async def log_assessment_completion(
    patient_id: str, assessment_type: str, provider_id: str
):
