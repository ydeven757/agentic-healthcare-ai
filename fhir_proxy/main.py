...
app = FastAPI(
    title="FHIR Proxy Service",
    description="Proxy service to handle FHIR requests with CORS support",
)
...
async def get_metadata(fhir_url: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(
            status_code=e.response.status_code, detail="FHIR server error"
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get(
                f"{target_url}/MedicationStatement?patient={patient_id}"
            )
            response.raise_for_status()
            return response.json()
    except httpx.RequestError as e:
...
async def get_patient_observations(patient_id: str, fhir_url: Optional[str] = None):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get(
                f"{target_url}/Observation?patient={patient_id}"
            )
            response.raise_for_status()
            return response.json()
    except httpx.RequestError as e:
...
async def search_medication_requests(request: Request, fhir_url: Optional[str] = None):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get(
                f"{target_url}/MedicationRequest", params=params
            )
            response.raise_for_status()
            return response.json()
    except httpx.RequestError as e:
